...
if [ -d ~/official_devices-gapps ]; then
  echo "Pulling the latest changes from the official_devices-gapps repository..."
...
if [ ! -d ~/official_devices-gapps ]; then
  cd ~
  git clone https://github.com/SuperiorOS-Devices/official_devices-gapps.git -b thirteen
fi
...
if [ ! -f ~/official_devices-gapps/$device.json ]; then
...
}" >~/official_devices-gapps/$device.json
  sleep 1
  echo "Done"
fi
...
if [ "$(cat ~/official_devices-gapps/$device.json | grep "device_name" | cut -d':' -f2 | cut -d'"' -f2)" == "Unknown" ]; then
  echo "Device name is not set, please enter it"
  read -p "Device name: " device_name
  device_name_old=$(cat ~/official_devices-gapps/$device.json | grep "device_name" | cut -d':' -f2 | cut -d'"' -f2)
  $(sed -i "s|$device_name_old|$device_name|g" ~/official_devices-gapps/$device.json)
fi

if [ "$(cat ~/official_devices-gapps/$device.json | grep "maintainer" | cut -d':' -f2 | cut -d'"' -f2)" == "Noob" ]; then
  echo "Maintainer is not set, please enter it"
  read -p "Maintainer: " maintainer
  maintainer_old=$(cat ~/official_devices-gapps/$device.json | grep "maintainer" | cut -d':' -f2 | cut -d'"' -f2)
  $(sed -i "s|$maintainer_old|$maintainer|g" ~/official_devices-gapps/$device.json)
fi
